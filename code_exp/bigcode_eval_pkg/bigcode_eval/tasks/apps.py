...
def create_all_tasks(split):
    """Creates a dictionary of tasks from a list of levels
    :return: {task_name: task}
        e.g. {apps-interview: Task, apps-competitoon: Task}
    """
    return {f"apps-{level}": create_task(level, split) for level in LEVELS}


def create_task(level, split):
    class APPS(GeneralAPPS):
        def __init__(self, **kwargs):
            super().__init__(level, split, **kwargs)
...
class GeneralAPPS(Task):
    ...
    def __init__(self, level, split, k_list=[1, 10, 100]):
