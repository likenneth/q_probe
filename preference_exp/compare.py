...
parser.add_argument('--sleep_time', '-st', help="how long to sleep to prevent rate limit hit", type=int, default=0.25)
...
parser.add_argument('--results_file', '-r', help="JSONL file to append to", type=str, default='samples/results.jsonl')
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.results_file):
        with open(args.results_file, 'r') as f1:
            results = [json.loads(line) for line in f1]
        if samples["config"]["exp_name"] in [r["exp_name"] for r in results]:
            print("@"*40)
            print("Already evaluated this experiment")
            print("@"*40)
            sys.exit()
    ...
    for batch in tqdm(samples["samples"]):
    ...
    # update the input file with winner for each sample
    json.dump(samples, open(args.file, 'w'), indent=2)
