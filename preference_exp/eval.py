...
def main(config: DictConfig):
    ...
    tokenizer_name_or_path = config.model.name_or_path # first see if saved tokenizer is in the experiment directory
    ...
    policy = transformers.AutoModelForCausalLM.from_pretrained(tokenizer_name_or_path, low_cpu_mem_usage=True, torch_dtype=policy_dtype, **model_kwargs)
    ...
    if config.mode == 'sample':
        ...
        if 1:
        ...
        elif 0:  # used for sample multiple answers for each prompt in test split
            eval_iterator = dataloader.SFTDataLoader(
                config.datasets, 
                tokenizer,
                split='test',
            ...
            trainer = BasicTrainer(tokenizer, config, None, eval_iterator, policy, reference_model=reference_model)
            samples = trainer.sample()
        else:  # uesd  only for train_extraction
            ...
            for batch in tqdm(eval_iterator):
                for prompt_text, chosen_text, rejected_text in zip(batch['prompt_text'], batch['chosen_text'], batch['rejected_text']):
                    samples.append({
                        'prompt' : prompt_text,
                        'chosen' : chosen_text,
                        'policy' : rejected_text,
                    })
